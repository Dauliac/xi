...
use xi_diff::{handle_nixos_diff, print_dix_diff};
use xi_remote::{self, RemoteBuildConfig, RemoteHost};
    ...
    match self.subcommand {
      ...
      OsSubcommand::Build(args) => {
        if args.common.ask || args.common.dry {
          warn!("`--ask` and `--dry` have no effect for `xi os build`");
        }
...
impl OsRebuildActivateArgs {
  ...
  fn rebuild_and_activate(
    ...
    {
      let guard = xi_remote::init_ssh_control();
    ...
    if xi_core::cache::is_push_configured(&self.rebuild.common.cache) {
      xi_core::cache::push_to_cache(&self.rebuild.common.cache, &out_path);
    }
  ...
  fn activate_rebuilt_config(
    ...
    if should_skip {
      warn!(
        "Skipping pre-activation validation (--no-validate or XI_NO_VALIDATE \
         set)"
      );
      warn!(
    ...
    if let Test | Switch = variant {
      if let Some(target_host) = &self.rebuild.target_host {
        let activation_type = match variant {
          Test => xi_remote::ActivationType::Test,
          Switch => xi_remote::ActivationType::Switch,
          #[allow(clippy::unreachable, reason = "Should never happen.")]
          _ => unreachable!(),
        };

        xi_remote::activate_remote(
          target_host,
          &resolved_profile,
          &xi_remote::ActivateRemoteConfig {
            platform: xi_remote::Platform::NixOS,
            activation_type,
    ...
    if let Boot | Switch = variant {
      if let Some(target_host) = &self.rebuild.target_host {
        xi_remote::activate_remote(
          ...
          &xi_remote::ActivateRemoteConfig {
            platform: xi_remote::Platform::NixOS,
            activation_type: xi_remote::ActivationType::Boot,
...
impl OsRebuildArgs {
  ...
  ///
  /// This must be called after [`xi_remote::open_ssh_control_master`]
  ...
  fn determine_remote_elevation(
    ...
    let uid = xi_remote::probe_remote_uid(target_host)?;
    Ok(uid != 0)
  }
  ...
  fn determine_output_path(
    ...
    if let Some(p) = self.common.out_link.clone() {
      ...
      let (path, guard) = if matches!(variant, BuildVm | BuildIso | Build) {
        ...
        let dir = tempfile::Builder::new().prefix("xi-os").tempdir()?;
  ...
  fn execute_build(
    ...
    // Extract flake ref and hostname for suggestions on failure
    ...
    if let Some(build_host) = self.build_host.clone() {
      ...
      let actual_store_path = result?;
      Ok(Some(actual_store_path))
    } else {
      // Local build - use the existing path
      let result = command::Build::new(toplevel)
        .extra_arg("--out-link")
        .extra_arg(out_path)
...
fn validate_system_closure_remote(
  ...
  xi_remote::validate_closure_remote(
    target_host,
    system_path,
    ESSENTIAL_FILES,
    context.as_deref(),
...
/// as `xi os` subcommands should not be run directly as root.
fn has_elevation_status(
  ...
  if is_root && !bypass_root_check {
    bail!(
      "Don't run xi os as root. It will escalate its privileges internally as \
       needed."
    );
  }
...
///
/// Delegates to [`resolve_toplevel`] with `nixosConfigurations` as the
/// config key and `["config", "system", "build"] ++ final_attrs` as the
/// build path.
///
/// # Errors
///
...
pub fn toplevel_for<S: AsRef<str>>(
  ...
  let mut build_path: Vec<&str> = vec!["config", "system", "build"];
  build_path.extend_from_slice(final_attrs);
  resolve_toplevel(
    hostname.as_ref(),
    installable,
