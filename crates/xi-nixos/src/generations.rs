...
impl Field {
  const fn column_info(&self, width: ColumnWidths) -> (&'static str, usize) {
    match self {
      ...
      Self::Never => ("NixOS Version", width.never),
...
pub fn print_info(
  ...
  let visible_fields: Vec<Field> = fields.map_or_else(
    || {
      use Field::{Confrev, Date, Id, Kernel, Never, Size, Spec};
      let all_fields = [Id, Date, Never, Kernel, Confrev, Spec, Size];
    ...
    let row: String = visible_fields
      ...
      .map(|f| {
        ...
        let cell_content = match f {
          ...
          Field::Never => generation.nixos_version.clone(),
