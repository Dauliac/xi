...
pub struct SearchArgs {
  ...
  /// Query shorthand: equivalent to `xi search packages <query>` or
  /// `xi search options <query>` depending on `--default-search`
...
impl SearchArgs {
  ...
  fn resolved_shorthand_mode(&self) -> Result<ResolvedSearchMode<'_>> {
    if self.query.is_empty() {
      bail!(
        "no query provided; try `xi search packages <query>`, `xi search \
         options <query>`, or `xi search --help`"
      );
    }
...
mod tests {
  ...
  fn parse_search(args: &[&str]) -> clap::error::Result<SearchArgs> {
    let cli = TestCli::try_parse_from(
      std::iter::once("xi").chain(args.iter().copied()),
    )?;
    match cli.command {
