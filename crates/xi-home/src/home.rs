    ...
    match self.subcommand {
      ...
      HomeSubcommand::Build(args) => {
        if args.common.ask || args.common.dry {
          warn!("`--ask` and `--dry` have no effect for `xi home build`");
        }
        args.rebuild(&Build)
      },
  ...
  fn rebuild(self, variant: &HomeRebuildVariant) -> Result<()> {
    ...
    let (out_path, _tempdir_guard): (PathBuf, Option<tempfile::TempDir>) =
      if let Some(ref p) = self.common.out_link {
        ...
        let dir = tempfile::Builder::new().prefix("xi-home").tempdir()?;
    ...
    let (suggest_ref, suggest_config) = toplevel.suggestion_context();
