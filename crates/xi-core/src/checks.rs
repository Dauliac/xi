...
pub fn verify_variables() -> Result<()> {
  if let Ok(f) = std::env::var("FLAKE") {
    // Set XI_FLAKE if it's not already set
    if std::env::var("XI_FLAKE").is_err() {
      unsafe {
        std::env::set_var("XI_FLAKE", f);
      }

      // Only warn if FLAKE is set and we're using it to set XI_FLAKE
      // AND none of the command-specific env vars are set
      if std::env::var("XI_OS_FLAKE").is_err()
        && std::env::var("XI_HOME_FLAKE").is_err()
        && std::env::var("XI_DARWIN_FLAKE").is_err()
      {
        tracing::warn!(
          "xi {} now uses XI_FLAKE instead of FLAKE, please update your \
           configuration",
          super::XI_VERSION
        );
      }
...
mod tests {
  ...
  fn test_setup_environment_flake_to_nh_flake_migration() {
    unsafe {
      ...
      env::remove_var("XI_OS_FLAKE");
      env::remove_var("XI_HOME_FLAKE");
      env::remove_var("XI_DARWIN_FLAKE");
    }
    ...
    assert!(
      result.is_ok(),
      "Should warn when migrating FLAKE to XI_FLAKE"
    );
    assert_eq!(
      env::var("XI_FLAKE").expect("XI_FLAKE should be set by test"),
      "/test/flake"
    );
  }
  ...
  fn test_setup_environment_no_migration_when_nh_flake_exists() {
    ...
    let _guard2 = EnvGuard::new("XI_FLAKE", "/existing/flake");

    let result = verify_variables();

    assert!(
      result.is_ok(),
      "Should not warn when XI_FLAKE already exists"
    );
    assert_eq!(
      env::var("XI_FLAKE").expect("XI_FLAKE should be set by test"),
      "/existing/flake"
  ...
  fn test_setup_environment_no_migration_when_specific_flake_vars_exist() {
    ...
    let _guard1 = EnvGuard::new("FLAKE", "/test/flake");
    let _guard2 = EnvGuard::new("XI_OS_FLAKE", "/os/flake");
  ...
  fn test_check_features_bypassed_with_nh_no_checks() {
    ...
    assert!(
      result.is_ok(),
      "check_features should succeed when XI_NO_CHECKS is set"
    );
  }
  ...
  fn test_verify_nix_environment_bypassed_with_nh_no_checks() {
    ...
    assert!(
      result.is_ok(),
      "verify_nix_environment should succeed when XI_NO_CHECKS is set"
    );
  }
  ...
  fn test_check_nix_version_bypassed_with_nh_no_checks() {
    ...
    assert!(
      result.is_ok(),
      "check_nix_version should succeed when XI_NO_CHECKS is set"
    );
  }
