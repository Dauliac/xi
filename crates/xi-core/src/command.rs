...
pub use nix_command::{CommandKind, NixCommand, find_real_nix_binary};
...
pub fn get_sudo_opts() -> Vec<String> {
  let sudoopts = env::var("XI_SUDOOPTS")
  ...
  shlex::split(&opts).unwrap_or_else(|| {
    warn!(
      "Failed to parse sudo opts from XI_SUDOOPTS/NIX_SUDOOPTS, ignoring. \
       Value: {opts}"
    );
    Vec::new()
  })
...
///
/// Defines how `xi` should handle privilege elevation for commands
...
///
/// This enum defines how `xi` should handle privilege elevation for commands
/// that require root access (e.g., `switch-to-configuration`).
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl Command {
  ...
  pub fn with_required_env(mut self) -> Self {
    ...
    const PRESERVE_ENV: &[&str] = &[
      ...
      // variables and xi continued to work, but any missing vars might
...
mod tests {
  ...
  fn test_with_required_env_nh_vars() {
    ...
    // Should include XI_* variables as Set actions
    assert!(
      matches!(cmd.env_vars.get("XI_TEST_VAR"), Some(EnvAction::Set(val)) if val == "test_value")
    );
    assert!(
  ...
  fn test_build_sudo_cmd_with_preserve_vars() {
    let _preserve_env_guard = EnvGuard::new("XI_PRESERVE_ENV", "1");
  ...
  fn test_build_sudo_cmd_with_preserve_vars_disabled() {
    let _preserve_env_guard = EnvGuard::new("XI_PRESERVE_ENV", "0");
    let _var1_guard = EnvGuard::new("VAR1", "1");
    let _var2_guard = EnvGuard::new("VAR2", "2");
  ...
  fn test_build_sudo_cmd_with_askpass() {
    let _guard = EnvGuard::new("XI_SUDO_ASKPASS", "/path/to/askpass");
  ...
  fn test_parse_cmdline_realistic_sudo() {
    let cmdline =
      r"/usr/bin/sudo env 'PATH=/path with spaces' /usr/bin/xi clean all";
  ...
  fn test_parse_cmdline_complex_sudo_command() {
    ...
    let cmdline = r#"/usr/bin/sudo -E env 'HOME=/root' "PATH=/usr/bin" /usr/bin/xi os switch"#;
    let result = shlex::split(cmdline).unwrap_or_default();
    assert_eq!(
      result,
      vec![
