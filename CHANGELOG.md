...
- `xi search offline <query>` subcommand for offline search using
  [spam-db](https://github.com/feel-co/spam) databases. Requires `-D <path>` (or
  `XI_OFFLINE_DB`) pointing to the database directory.
- `xi search prs <query>` subcommand for searching recent Nixpkgs pull requests
  and showing which Nixpkgs branches merged PRs have reached. Numeric queries
  and `#<number>` fetch that pull request directly.
- `xi search issues <query>` subcommand for searching recent Nixpkgs issues
  while excluding pull requests.
- `xi search prs` reads GitHub authentication from `GH_TOKEN` first, then checks
  `$XDG_STATE_HOME/xi/github-token` for a token file, falling back to
  `~/.local/state/xi/github-token`.
- `xi search --default-search` global option to set default search mode
  (`packages` or `options`) when no subcommand is specified.
- `xi clean --keep-one` preserves all active direnv gcroots regardless of
...
- `xi clean --cross-filesystems` / `-x` allows the gcroot scan to cross
  filesystem boundaries. By default the walk stays on the same filesystem as
  `/nix/var/nix/gcroots`.
- `--no-direnv` passed to `xi clean all` will now preserve `.direnv/` paths as
  well.
...
- `xi os {boot,build,switch}` now supports diffing on remote machines.
- xi now has an in-house `nix-command` crate, which is used for building typed
  nixos commands. `nix-command` is also
  [published](https://crates.io/crates/nix-command) to crates.io.

### Changed

- **Breaking Change**: `xi search` CLI has been restructured to use subcommands.
  - `--options <query>` is now `xi search options <query>`
...
- `xi search` now uses search backend version 48 (previously 46) to track the
...
- Explicit local flake refs are now checked by xi before Nix runs. Local flake
...
- Empty/malformed flake refs are now rejected early. Exempli gratia,
  `XI_FLAKE="" xi os switch`, `xi os repl ''`, and `xi os switch '#attr'` no
  longer work.
- `XI_FLAKE` now also does flake feature detection. A command resolving its
  installable from `XI_FLAKE` now also checks for the required Nix experimental
  features.
...
- Generated Nushell completions now mark the `installable` argument as a path
  while keeping xi's own parser compatible with flake references.
- `xi os switch --target-host root@host` no longer wraps the activation in
...
- `xi os info` now batches `nix path-info` calls into a single invocation
  ...
  ([#636](https://github.com/Dauliac/xi/issues/636))
  - `xi os info` now uses a direct map lookup when extracting closure sizes from
...
- The `indicatif` spinner shown during remote closure copying has been fixed so
  ...
  ([#635](https://github.com/Dauliac/xi/pull/635))

### Removed

- `xi` no longer supports `x86_64-darwin`, following Nixpkgs' decision to drop
...
- `xi os rollback` performance has been improved.
...
- Specialisations now get correctly installed when running xi os {switch, boot}
  with a specialisation selected.
- The environment variable `XI_SHOW_ACTIVATION_LOGS` now supports `1` (and
...
- A new `xi os build-image` subcommand for building a disk-image variant is now
...
- `--elevation-program` flag was renamed to `--elevation-strategy` with support
  ...
  ([#434](https://github.com/Dauliac/xi/issues/434))
  - Multi-program remote elevation support: `sudo`, `doas`, `run0`, and `pkexec`
    are now supported with correct flags for each program
  - Environment variable `XI_ELEVATION_PROGRAM` is still supported for backward
    compatibility (falls back to `XI_ELEVATION_STRATEGY` if set)
- Platform commands (`xi os`, `xi home`, `xi darwin`) now support SSH-based
  ...
  ([#428](https://github.com/Dauliac/xi/issues/428),
...
- Shell argument splitting now uses `shlex` for proper quote handling in complex
  command arguments.
- `xi os info` now supports `--fields` to select which field(s) to display
  ([#375](https://github.com/Dauliac/xi/issues/375)).
  - Empty columns are now hidden by default to avoid visual clutter.
  - A new, per-generation "Closure Size" column has been added
- `xi os switch` and `xi os boot` now support the `--install-bootloader` flag,
  ...
  explicitly passing the variable to `xi` commands, which has now been made
  explicit through the `--install-bootloader` flag.
  ([#424](https://github.com/Dauliac/xi/issues/424))
- A `--run` flag was added to `xi os build-vm`, which allows immediately
...
- Switched from owo-colors to Yansi as the internal coloring library. This
  ...
  different.
- `xi os info` now hides empty fields by default, they can be explicitly shown
  via the `--fields` flag.
- `xi completions` now supports [nushell](https://www.nushell.sh/)
- Platform commands (`xi os`, `xi home`, `xi darwin`) now accept a
  ...
  - The flag can be set globally via the `XI_SHOW_ACTIVATION_LOGS` environment
    variable.
  - `xi search` displays a link to the `package.nix` file on the nixpkgs GitHub,
...
- Direnv caches in [alternative locations][direnv-alternative-caches] (e.g.,
  `$XDG_CACHE_DIR/direnv/layouts`) will now be detected during `xi clean`.
- Fixed `--use-substitutes` being incorrectly passed to `nix build`, causing
  "unrecognised flag" errors.
- xi now properly resolves installables, fixing issues when e.g. multiple
  `XI_{FLAKE,FILE,{OS,HOME_DARWIN}_FLAKE}` environment variables are set.
...
- `xi darwin switch` now shows the output from the `darwin-rebuild` activation.
...
- `xi clean` will now skip directories that are checked and don't exist. Instead
  of throwing an error, it will print a warning about which directories were
  skipped.
- xi's verbosity flag can now be passed multiple times for more verbose debug
  output.
- `xi search` will now use the system trust store for it's HTTPS requests.
- Error handling has been improved across the board, with more contextual errors
  replacing direct error propagation or unwraps.
- The directory traversal during `xi clean` has been improved slightly and
...
- Nh will now carefully pick environment variables passed to individual
  ...
  ([#314](https://github.com/Dauliac/xi/issues/314))
- Fixed a tempdir race condition causing activation failures.
  [#386](https://github.com/Dauliac/xi/pull/386)
...
- The Elasticsearch backend version has been updated to v43, which fixes failing
  search commands ([#316](https://github.com/Dauliac/xi/pull/316))

## 4.1.0

### Added

- A new `xi os rollback` subcommand has been added to allow rolling back a
...
- Fix `--configuration` being ignored in `xi home switch`
  ([#262](https://github.com/Dauliac/xi/issues/262))

## 4.0.2

### Added

- Add `--json` to `xi search`, which will return results in JSON format. Useful
  for parsing the output of `xi search` with, e.g., jq.
