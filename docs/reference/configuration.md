...
- **`config.toml`** — user-level settings (build defaults, cache, develop daemon)
...
When using the NixOS/Home Manager/flake-parts module with wrapper enabled,
the config file is generated as a store path and `XI_CONFIG` is set
automatically.
...
Located at the root of your flake directory. Controls per-project behaviour
for CI, doctor, formatting, testing, and materialization.
